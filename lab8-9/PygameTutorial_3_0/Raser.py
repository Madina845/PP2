...
SCORE = 0
COINS_COLLECTED = 0
N = 5  # Кол-во монет для увеличения скорости
...
# Coin class with random weight and size
class Coin(pygame.sprite.Sprite):
    ...
    def reset(self):
        # Случайное значение монеты
        self.value = random.choice([1, 2, 3])

        # Установка размера в зависимости от ценности
        size = 20 + (self.value - 1) * 10  # 20, 30, 40
...
while True:
    ...
    for coin in coins:
        if pygame.sprite.collide_rect(P1, coin):
            COINS_COLLECTED += coin.value  # прибавляем ценность монеты
            coin.reset()

            # Увеличиваем скорость врагов при каждом N-м очке
